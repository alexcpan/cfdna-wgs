...
cfdna_wgs_libraries = pd.read_table(config["datadir"] + "/inputs/cfdna_wgs-5_3_23.clean.tsv")

readable = []
FASTQ_PREFIX = "/data/panalc/FASTQ/seq_libs/"
cfdna_wgs_libraries["file"] = FASTQ_PREFIX + cfdna_wgs_libraries["basename"]
for x in cfdna_wgs_libraries["file"]:
    readable.append(os.access(x, os.R_OK))
# Ensure readable fastqs
cfdna_wgs_libraries['readable']=readable
cfdna_wgs_libraries = cfdna_wgs_libraries[cfdna_wgs_libraries.readable == True]
# Ensure correct library type per sample sheet
cfdna_wgs_libraries = cfdna_wgs_libraries[cfdna_wgs_libraries["seq"] == "wgs"]
cfdna_wgs_libraries = cfdna_wgs_libraries[cfdna_wgs_libraries["isolation"] == "cfdna"]
...
print(CFDNA_WGS_HEALTHY_LIBRARIES)

old_healthies = []
for lib in CFDNA_WGS_HEALTHY_LIBRARIES:
    if int(lib[3:])<250:
        old_healthies.append(lib)
for lib in old_healthies:
    CFDNA_WGS_HEALTHY_LIBRARIES.remove(lib)
...
rule all:
    input:
        ...
        # expand(cfdna_wgs_frag_gc_distros + "/{library}_gc_distro.csv", library = CFDNA_WGS_HEALTHY_LIBRARIES),
        # expand(cfdna_wgs_frag_beds + "/{library}_sampled_frag.bed", library = CFDNA_WGS_HEALTHY_LIBRARIES),
